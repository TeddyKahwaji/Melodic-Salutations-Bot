...
class DeleteVoicelineView(discord.ui.View):
    def __init__(self, voice_lines: list[str], member: discord.Member, voice_line_type: str, requester_member, delete_key, insert_key):
        self.member = member
    ...
    async def send(self, interaction: discord.Interaction):
        self.selection_wheel.options = [discord.SelectOption(
            label=voiceline, value=voiceline) for voiceline in self.voice_lines[:25]]
    ...
    async def selection_wheel(self, interaction, select: discord.Interaction):
        ...
        data = {
            key:  firestore.firestore.ArrayRemove([filename for _, filename in results]),
